...
import {getQuery, getRequestHeader, getRouterParam, setHeader} from 'h3'
...
export default defineCachedEventHandler(async (event) => {
  ...
  if(getCookie(event, 'lw_token')?.length ?? 0) {
    const authHeader = getRequestHeader(event, 'authorization')
    const headers = authHeader ? { authorization: authHeader } : undefined

    event.context._nitro.skipCache = true

    try {
      return await $fetch<ApiResp<Pagination<CommentDto>>>(upstream, {
        query,
        headers,
      })
    } catch (err: any) {
