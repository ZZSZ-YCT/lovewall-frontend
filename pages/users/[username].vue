...
// SEO
useHead(() => {
  if (loading.value) return { title: '加载中... - 郑州四中表白墙' }
  if (error.value) return { title: '用户不存在 - 郑州四中表白墙' }
  if (user.value) {
    const name = user.value.display_name || user.value.username
    return {
      title: `${name} (@${user.value.username}) - 郑州四中表白墙`,
      meta: [
        { name: 'description', content: `查看 ${name} 在郑州四中表白墙的个人主页` },
      ]
    }
  }
